...
BEGIN
    ...
    IF (vote_exists = False) THEN
        ...
        UPDATE thread SET thread_votes = thread_votes + voice_param WHERE thread_number = thread_id;
    END IF;
    IF (vote_exists = True) THEN
        IF (x != voice_param) THEN
            UPDATE vote SET vote_voice = voice_param WHERE LOWER(vote_nickname) = LOWER(nickname_param) AND thread_number = vote_thread_id;
            UPDATE thread SET thread_votes = thread_votes - x + voice_param  WHERE thread_number = thread_id;
        END IF;
    END IF;
    RETURN 'VOTE_OK';
END;
